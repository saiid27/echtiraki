      ...
      <main style={{
        ...
        {products.map(p=>(
          <div key={p.id} style={{background:'#fff', border:'1px solid #eee', borderRadius:10, overflow:'hidden'}}>
            <img src={p.cover} alt={p.name} style={{width:'100%', height:150, objectFit:'contain', background:'#f6f6f6'}}/>
            <div style={{padding:12}}>
              <b>{p.name}</b>
              <div style={{color:'#666', margin:'6px 0'}}>{p.short}</div>
              <div style={{margin:'8px 0'}}><b>${(p.priceCents/100).toFixed(2)}</b> / شهر</div>
