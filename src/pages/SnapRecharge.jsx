// ==========================
// File: src/pages/SnapRecharge.jsx
// Description: صفحة شحن سناب شات بلس — بنفس أسلوب وهيكلة Contact.jsx
// - نفس منطق التوست، التحقق، واتساب/mailto، وCSS مضمّن داخل الصفحة
// - RTL افتراضيًا
// ==========================
...
const PRICE_MONTH = Number(import.meta.env.VITE_SNAP_PRICE_MONTH ?? 130);
const PRICE_YEAR  = Number(import.meta.env.VITE_SNAP_PRICE_YEAR  ?? 1200);
...
export default function SnapRecharge() {
  ...
  // أدوات
  const planPrice = (key) => (key === "year" ? PRICE_YEAR : PRICE_MONTH);
  const planLabel = (key) => (key === "year" ? "12 شهر" : "شهر واحد");
  const fmtMRU = (v) => {
    try { return new Intl.NumberFormat("ar-MR", { style: "currency", currency: "MRU" }).format(v); }
    catch { return `${v} MRU`; }
  };
  ...
  function validate() {
    ...
    if (!username.trim() || !plan) {
      showToast("يرجى تعبئة اسم المستخدم واختيار الباقة *");
      return false;
    }
    if (!/^([a-zA-Z0-9._-]{2,30})$/.test(username.trim())) {
      showToast("اسم المستخدم غير صالح (حروف/أرقام ونقاط وشرطة)");
      return false;
    }
      ...
      <div className="wrap">
        <div className="card" role="region" aria-label="شحن سناب شات بلس">
          <div className="header">
            ...
            <div className="title">
              <h1>شحن سناب شات بلس</h1>
              <p>باقات رسمية وسريعة — الدفع عبر واتساب أو البريد أو تحويل بنكي.</p>
            </div>
          </div>
          ...
          <div className="grid">
            ...
            <form className="panel" onSubmit={sendMail} noValidate>
              <div className="row">
                <div>
                  <label htmlFor="username">اسم مستخدم سناب <span className="req">*</span></label>
                  <input
                    id="username"
                    placeholder="مثال: snap.username"
                    value={form.username}
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                  >
                    <option value="month">{`شهر واحد — ${fmtMRU(PRICE_MONTH)}`}</option>
                    <option value="year">{`12 شهر — ${fmtMRU(PRICE_YEAR)}`}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="row">
                ...
                <div>
                  <label htmlFor="region">الدولة/المنطقة</label>
                  <input
                    id="region"
                    placeholder="مثال: موريتانيا"
                    value={form.region}
              ...
              <div className="row">
                <div>
                  <label htmlFor="email">بريد للتواصل (اختياري)</label>
                  <input
                    id="email"
                    type="email"
                    inputMode="email"
                ...
                <div>
                  ...
                  <input
                    id="notes"
                    placeholder="مثال: تفعيل فوري، وقت مفضل للاتصال..."
                    value={form.notes}
              ...
              <div className="list">
                <div className="item">
                  ...
                  <div>
                    ...
                    <div className="small">{`شهر: ${fmtMRU(PRICE_MONTH)} • سنة: ${fmtMRU(PRICE_YEAR)}`}</div>
                  </div>
                </div>
                <div className="item">
                  <div className="ico">⚡</div>
                ...
                <div className="item">
                  ...
                  <div>
                    ...
                    <div className="small">الأحد–الخميس • 9ص–5م</div>
                  </div>
                </div>
              </div>
            </aside>
