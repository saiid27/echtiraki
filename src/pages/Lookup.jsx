...
// ضع رابط Google Apps Script هنا مباشرة
...
// ضع في .env: VITE_GAS_LOOKUP_URL="https://script.google.com/macros/s/....../exec"
...
export default function Lookup() {
  ...
  async function onSubmit(e) {
    ...
    setResult(null);
    if (!GAS_URL) {
      setError("لم يتم ضبط رابط خدمة Google Apps Script.");
      return;
    }
    ...
    try {
      ...
      if (!json.ok) {
        const msg = json.error === "not_found" ? "لم يتم العثور على طلب بهذه البيانات." :
                    json.error === "missing_params" ? "بيانات ناقصة." :
                    "حدث خطأ أثناء الاستعلام.";
        setError(msg);
      } else {
  ...
  return (
    <article className="page">
      ...
      <form className="form" onSubmit={onSubmit} style={{marginTop:12}}>
        <label>
          الاسم
          <input value={name} onChange={(e)=>setName(e.target.value)} placeholder="مثال: Mohamed" />
        </label>
        <label>
          الرقم السري
          <input value={code} onChange={(e)=>setCode(e.target.value)} placeholder="مثال: NX-4K-AB12" />
        </label>
      ...
      {result && (
        <div className="form" style={{marginTop:12}}>
          ...
          <p><strong>المنتج:</strong> {result.product}</p>
          <p><strong>المدة:</strong> {result.duration}</p>
          <p><strong>السعر:</strong> {result.price} MRU</p>
          {result.status && <p><strong>الحالة:</strong> {result.status}</p>}
